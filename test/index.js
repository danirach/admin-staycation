...
describe('API END POINT TESTING',()=>{
    ...
    it('GET Detail Page', (done)=>{
        chai.request(app).get('/api/v1/member/detail-page/5e96cbe292b97300fc902222').end(
            (err,res)=>{
                expect(err).to.be.null
                expect(res).to.have.status(200)
                expect(res.body).to.be.an('object')
    ...
    it('POST Booking Page', (done)=>{
        ...
        const dataSample = {
                ...
                duration:'2',
                bookingStartDate:'2022-06-20',
                bookingEndDate:'2022-06-22',
                firstName:'agus',
                lastName:'hari',
                email:'agushari22@gmail.com',
                phoneNumber:'082193832928',
                accountHolder:'Agus',
                bankFrom:'BRI',
        }
        chai.request(app).post('/api/v1/member/booking-page')
        .set('Content_Type', 'application/x-www-form-url-encoded')
